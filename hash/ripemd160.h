...
public:
    CRIPEMD160() noexcept;
    void Write(const unsigned char* data, size_t len) noexcept;
    void Finalize(unsigned char hash[20]) noexcept;
};
...
void ripemd160sse_test() noexcept;
std::string ripemd160_hex(unsigned char* digest) noexcept;

static inline bool ripemd160_comp_hash(uint8_t* h0, uint8_t* h1) noexcept
{
    const uint32_t* h0i = (const uint32_t*)h0;
    const uint32_t* h1i = (const uint32_t*)h1;
    return (h0i[0] == h1i[0]) &&
           (h0i[1] == h1i[1]) &&
           (h0i[2] == h1i[2]) &&
           (h0i[3] == h1i[3]) &&
