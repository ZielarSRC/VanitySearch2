...
// Optimized rotation macro using SSE intrinsics
#define ROL(x, n) _mm_or_si128(_mm_slli_epi32(x, n), _mm_srli_epi32(x, 32 - (n)))

// Define logical functions for RIPEMD-160
#ifdef _MSC_VER
...
// Round function macros with improved instruction scheduling
#define Round(a,b,c,d,e,f,x,k,r) \
  do { \
    __m128i u = add4(a,f,x,_mm_set1_epi32(k)); \
...
// Optimized word loading with better memory access pattern
#define LOADW(i) _mm_setr_epi32( \
...
void Initialize(__m128i *s) {
    ...
    _mm_store_si128(s + 0, _mm_load_si128((const __m128i*)(_init + 0)));
    _mm_store_si128(s + 1, _mm_load_si128((const __m128i*)(_init + 4)));
    _mm_store_si128(s + 2, _mm_load_si128((const __m128i*)(_init + 8)));
    _mm_store_si128(s + 3, _mm_load_si128((const __m128i*)(_init + 12)));
    _mm_store_si128(s + 4, _mm_load_si128((const __m128i*)(_init + 16)));
}

// Perform 4 RIPE in parallel using SSE2 with optimized scheduling
void Transform(__m128i *s, uint8_t *blk[4]) {
    ...
    // Finalize state
    __m128i t = _mm_load_si128(s + 0);
    _mm_store_si128(s + 0, add3(_mm_load_si128(s + 1), c1, d2));
    _mm_store_si128(s + 1, add3(_mm_load_si128(s + 2), d1, e2));
    _mm_store_si128(s + 2, add3(_mm_load_si128(s + 3), e1, a2));
    _mm_store_si128(s + 3, add3(_mm_load_si128(s + 4), a1, b2));
    _mm_store_si128(s + 4, add3(t, b1, c2));
}

} // namespace ripemd160sse

// Output packing macros
#ifdef _MSC_VER
#define DEPACK(d,i) \
...
// Optimized SSE implementation for 32-byte inputs
void ripemd160sse_32(
    ...
    // Initialize and process the blocks
    ripemd160sse::Initialize(s);
    
    // Prepare padding and length for all blocks
    for (int i = 0; i < 4; i++) {
